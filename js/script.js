...
function mostrarResumenCarrito() {
    ...
    let resumen = `<div class="carrito-resumen" style="text-align: left;">`;
    resumen += `<h3>🛒 Resumen del Carrito</h3>`;
    
    productosEnCarrito.forEach((producto, index) => {
        resumen += `
            <div style="display: flex; justify-content: space-between; align-items: center; margin: 15px 0; padding: 12px; border: 1px solid #ddd; border-radius: 8px; background: #f8f9fa;">
                <div style="flex: 1;">
                    <strong style="color: #333;">${producto.nombre}</strong><br>
                    <small style="color: #666;">Cantidad: ${producto.cantidad} | Precio unitario: $${producto.precio.toLocaleString()}</small>
                </div>
                <div style="display: flex; align-items: center; gap: 15px;">
                    <span style="font-weight: bold; color: #28a745; font-size: 16px;">$${(producto.precio * producto.cantidad).toLocaleString()}</span>
                    <button onclick="eliminarProductoDelCarrito(${producto.id})" 
                            style="background: #ff6b6b; color: white; border: none; padding: 8px 12px; border-radius: 5px; cursor: pointer; font-size: 14px;">
                        🗑️ Eliminar
                    </button>
                </div>
            </div>
    ...
    resumen += `<hr style="margin: 20px 0; border: 2px solid #87c3bd;">`;
    resumen += `<div style="text-align: center; font-size: 20px; font-weight: bold; color: #333; background: #87c3bd; padding: 15px; border-radius: 8px;">
...
function eliminarProductoDelCarrito(productId) {
    const index = productosEnCarrito.findIndex(p => p.id === productId);
...
function vaciarCarrito() {
    Swal.fire({
        ...
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: '🗑️ Sí, vaciar carrito',
